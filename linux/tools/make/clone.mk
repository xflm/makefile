...
CLONE_ALL_FILE = $(sort $(filter-out %.d,$(MAKEFILE_LIST))) $(SRC_FILE) \
$(ASM_FILE) $(filter-out $(BUILD_DIR)/%, $(LIB_FILE)) $(LINK_FILE) $(OTHER_FILE)
CLONE_TARGET_DIR = ../$(TARGET)-clone
FILE_MK = $(filter %/file.mk, $(MAKEFILE_LIST))
TARGET_FILE_MK = $(addprefix $(CLONE_TARGET_DIR)/, $(FILE_MK))
...
ifeq ($(wildcard  $(TARGET_ELF) $(TARGET_LIB)),)
	...
	@ALL_FILE=$$(echo "$(CLONE_ALL_FILE)" | tr ' ' '\n' | sed "s|^\./||g;/^$$/d"); \
     ...
     for i in $$LINUX_ABS_FILE; do j=$$(grep $$i $$FILE_MK); \
         if [ -z "$$j" ]; then m=$${i#/}" "; \
     ...
     for i in $$WIN_ABS_FILE; do j=$$(grep $$i $$FILE_MK); i=/$${i/:/}; \
         if [ -z "$$j" ]; then m=$${i#/}" "; \
         CLONE_ABS_FILE+=$$CLONE_TARGET_DIR/$$m; MK_ABS_FILE+=$$m; \
         else j=$${j%%/file.mk*}; m=$${j##*/}; m=$${i##*/$$m/}" "; \
         CLONE_ABS_FILE+=$$CLONE_TARGET_DIR/$$j/$$m; \
         MK_ABS_FILE+='$$(FILE_PWD)/'$$m; fi; done;\
     for i in $$HOME_ABS_FILE; do j=$$(grep $$i $$FILE_MK); i=$${i/\~/$$HOME}; \
         if [ -z "$$j" ]; then m=$${i#/}" "; \
         CLONE_ABS_FILE+=$$CLONE_TARGET_DIR/$$m; MK_ABS_FILE+=$$m; \
         else j=$${j%%/file.mk*}; m=$${j##*/}; m=$${i##*/$$m/}" "; \
         CLONE_ABS_FILE+=$$CLONE_TARGET_DIR/$$j/$$m; \
     ...
     for i in $$LINUX_HEADER_PATH; do j=$$(grep $$i $$FILE_MK); \
         if [ -z "$$j" ]; then m=$${i#/}; \
     ...
     for i in $$WIN_HEADER_PATH; do j=$$(grep $$i $$FILE_MK); n=/$${i/:/}; \
         if [ -z "$$j" ]; then m=$${n#/}; \
     ...
     for i in $$HOME_HEADER_PATH; do j=$$(grep $$i $$FILE_MK); n=$${i/\~/$$HOME}; \
         if [ -z "$$j" ]; then m=$${n#/}; \
     ...
     for i in $$CLONE_NORMAL_FILE $$CLONE_TOP_RELATIVE_FILE $$CLONE_RELATIVE_FILE \
         ...
         $$CLONE_LINUX_ABS_HEADER $$CLONE_WIN_ABS_HEADER $$CLONE_HOME_ABS_HEADER; do \
         CLONE_PATH+=$${i%/*}" "; done; \
     mkdir -p $$CLONE_PATH; \
     ...
     for i in $$TARGET_FILE_MK; do j=$${i%/file.mk}; m=$${j##*/}; \
         sed -i "s|\.\..*/$$m/||g" $$i; done; \
     ...
     for ((i = 0; i < $${#ABS_NAME[@]}; i++)); do \
         sed -i "s|$${ABS_NAME[$$i]}|$${NEW_NAME[$$i]}|g" $$TARGET_FILE_MK $$CLONE_TARGET_DIR/file.mk; done; \
     ...
     for ((i = 0; i < $${#NEW_LIB_MK[@]}; i++)); do \
         ...
         LINE1=$$(grep FILE_LIB -n $$TMP1 | head -1 | cut -d':' -f1); \
         sed -i "$$LINE1""iFILE_LIB += $$""(FILE_PWD)/$${NEW_LIB_NAME[$$i]}" $$TMP1; done; \
     ...
     sed -i "s|$$""(G_TOOLCHAIN)|$$G_TOOLCHAIN|g;s/$$""(G_BUILD_TYPE)/$$G_BUILD_TYPE/g; \
         s/$$""(G_CPU)/$$G_CPU/g;s/$$""(G_INCFLAGS)//g;s/$$""(LIB_NAME)/$$TARGET/g;\
         s/$$""(G_DEFINEFLAGS)/$$G_DEFINEFLAGS/g;/OBJ_DIR/d" Makefile; \
